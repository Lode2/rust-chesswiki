...
pub fn moves(pos: &Position) -> Vec<&str> {
...
fn pseudo_legal_moves(pos: &Position, moves: &mut Vec<&str>) {
    ...
    // test loop
    for i in pos.get_pieces(pos.state.stm).into_iter() {
        if i.0 == 0 {
...
fn add_plegal_pawn_push(moves: &mut Vec<&str>, pos: &Position, piece: (usize, usize, usize)) {
    /*
...
fn add_plegal_pawn_capture(moves: &mut Vec<&str>, pos: &Position, piece: (usize, usize, usize)) {
    /*
    ...
    */
    ...
    if !on_a_file {
        ...
        if front_left_occupied {
            // let move_name = &("ax".to_owned() + bb_idx_to_square_id(front_left_square));
            moves.push(a_lifetime_str("ax", bb_idx_to_square_id(front_left_square)).as_str());
            ...
            if en_passant_front_left_occupied {
                moves.push(bb_idx_to_square_id(front_left_square));
            }
        }
    }
    ...
    if !on_h_file {
        ...
        if front_right_occupied {
            moves.push(bb_idx_to_square_id(front_right_square));
        } else if en_passant_option != None {
            if !en_passant_front_left_occupied {
                let en_passant_u64 = en_passant_option.unwrap();
...
fn add_plegal_bishop_move(moves: &mut Vec<&str>, pos: &Position, piece: (usize, usize, usize)) {}
fn add_plegal_knight_move(moves: &mut Vec<&str>, pos: &Position, piece: (usize, usize, usize)) {}
fn add_plegal_rook_move(moves: &mut Vec<&str>, pos: &Position, piece: (usize, usize, usize)) {}
fn add_plegal_queen_move(moves: &mut Vec<&str>, pos: &Position, piece: (usize, usize, usize)) {}
fn add_plegal_king_move(moves: &mut Vec<&str>, pos: &Position, piece: (usize, usize, usize)) {}

fn bb_idx_to_square_id<'a>(idx: usize) -> &'a str {
    return match idx {
        0 => "a1",
        1 => "b1",
        2 => "c1",
...
fn a_lifetime_str<'a>(str1: &str, str2: &str) -> String {
